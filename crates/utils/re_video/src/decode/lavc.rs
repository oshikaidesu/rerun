...
fn open_decoder(
    ...
    if hw != DecodeHardwareAcceleration::PreferSoftware {
        if let Some(device) = open_hw_device() {
            ...
            return Err(DecodeError::Lavc(
                "no hardware decoder device available".to_owned(),
            ));
        }
    }
    let video = context
...
impl LavcDecoder {
    pub fn new(
        ...
        Ok(Self {
            ...
            threads_ahead: std::thread::available_parallelism()
                .map_or(4, |n| n.get())
                .min(8),
            drained: false,
            hurry: false,
    ...
    fn reopen(&mut self) {
        match open_decoder(&self.codec, self.hw) {
            ...
            Err(err) => re_log::warn_once!(
                "{}: could not reopen libavcodec decoder: {err}",
                self.debug_name
            ),
        }
        self.drained = false;
    ...
    fn frame_out(&mut self) -> FrameResult {
        ...
        let source: &ff::util::frame::Video = if self.frame.format()
            == ff::format::Pixel::VIDEOTOOLBOX
            ...
            || self.frame.format() == ff::format::Pixel::VAAPI
        {
            ...
            let rc = unsafe {
                ff::ffi::av_hwframe_transfer_data(
        ...
        let (layout, planes): (YuvPixelLayout, &[(usize, usize, usize)]) = match source.format() {
            ...
            other => {
                return Err(DecodeError::Lavc(format!(
            ...
            _ => {
                matches!(
                    ...
                    ff::format::Pixel::YUVJ420P
                        | ff::format::Pixel::YUVJ422P
                        | ff::format::Pixel::YUVJ444P
        ...
        let coefficients = match source.color_space() {
            ...
            _ => self
                .yuv_hint
                .map_or(YuvMatrixCoefficients::Bt709, |(_, c)| c),
        };
