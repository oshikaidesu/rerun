...
fn all_wgsl_shaders_are_valid() {
    ...
    for shader in &shaders {
        ...
        let defaults = [re_renderer::renderer::DEFAULT_FIELD, re_renderer::renderer::DEFAULT_MOTION, surface];
        for hook in ["program_field", "program_motion", "program_tint", "program_surface"] {
            let defined = source.lines().any(|line| line.starts_with(&format!("fn {hook}(")));
