    ...
    for c in contours {
        ...
        for i in 0..edges {
            ...
            if v0.out_tangent == glam::Vec2::ZERO && v1.in_tangent == glam::Vec2::ZERO {
                ...
                path.curve_to(
                    at(v0.point + v0.out_tangent),
                    at(v1.point + v1.in_tangent),
                    at(v1.point),
                );
            }
        }
    ...
    for c in contours {
        ...
        for i in 0..n {
            ...
            cubic.for_each_quadratic_bezier(tolerance, &mut |q| {
                out.push([
                    glam::vec2(q.from.x, q.from.y),
                    glam::vec2(q.ctrl.x, q.ctrl.y),
                    glam::vec2(q.to.x, q.to.y),
                ]);
            });
        }
    }
...
impl PathDrawDataBuilder {
    ...
    pub fn into_mesh(self, ctx: &RenderContext, label: &str) -> crate::mesh::CpuMesh {
        let mut positions: Vec<glam::Vec3> = self
            .vertices
            .iter()
            .map(|v| glam::Vec2::from(v.position).extend(0.0))
            .collect();
        let mut colors: Vec<Rgba32Unmul> =
            self.vertices.iter().map(|v| Rgba32Unmul(v.color)).collect();
        let mut texcoords: Vec<glam::Vec2> =
            self.anchors.iter().map(|a| glam::Vec2::from(*a)).collect();
        let mut triangles: Vec<glam::UVec3> = self
            .indices
            .chunks_exact(3)
            .map(|i| glam::uvec3(i[0], i[1], i[2]))
            .collect();
        let ramps: std::cell::RefCell<
            Vec<(
                *const crate::mesh::CurveGradient,
                crate::resource_managers::GpuTexture2D,
            )>,
        > = Default::default();
        let material =
            |range: std::ops::Range<u32>,
             curves: Option<std::sync::Arc<crate::mesh::CurveFill>>| {
        ...
        for (fill, color) in self.curve_fills {
            let (lo, hi) = fill.curves.iter().flatten().fold(
    ...
    fn push(
        ...
        self.anchors
            .extend(buffers.vertices.iter().map(|(_, a)| *a));
        self.indices
    ...
    pub fn fill_exact_gradient(
        &mut self,
        contours: &[PathContour],
        rule: PathFillRule,
        gradient: crate::mesh::CurveGradient,
    ) {
    ...
    fn push_exact(
        ...
        contours: &[PathContour],
        rule: PathFillRule,
        gradient: Option<std::sync::Arc<crate::mesh::CurveGradient>>,
        color: Rgba32Unmul,
    ) {
        let pieces: Vec<(Vec<[glam::Vec2; 3]>, glam::Vec2, glam::Vec2)> = contours
            .iter()
        ...
        let mut merged: std::collections::BTreeMap<usize, Vec<[glam::Vec2; 3]>> =
            std::collections::BTreeMap::new();
        for (i, (curves, _, _)) in pieces.into_iter().enumerate() {
            merged
                .entry(root(&mut group, i))
                .or_default()
                .extend(curves);
        }
        for curves in merged.into_values() {
            self.curve_fills.push((
                crate::mesh::CurveFill {
                    curves,
                    even_odd: rule == PathFillRule::EvenOdd,
    ...
    pub fn fill(
        ...
        if let Some(buffers) = fill_buffers(contours, rule, self.tolerance.unwrap_or(TOLERANCE)) {
            let anchored = VertexBuffers {
                vertices: buffers.vertices.iter().map(|p| (*p, *p)).collect(),
...
mod exact_fill_tests {
    ...
    fn separate_shapes_become_separate_pieces_and_holes_stay_with_their_outline() {
        ...
        builder.fill_exact(
            &[
                square(glam::Vec2::ZERO, 10.0),
                square(glam::vec2(20.0, 0.0), 10.0),
            ],
            PathFillRule::NonZero,
            Rgba32Unmul([0, 0, 0, 255]),
        );
